...
const userLogin = (data) => {
  return data.loginSchema.validate(data, options);
};
...
const checkEmailOrPhone = async (data) => {
  if (validateEmail(data.emailOrPhone)) {
    await userLogin({
      email: data.emailOrPhone,
      password: data.password,
      loginSchema: loginWithEmailSchema
    })
    return 'email';
  }
  else if (validatePhoneNumber(data.emailOrPhone)) {
    await userLogin({
      email: data.emailOrPhone,
      password: data.password,
      loginSchema: loginWithMobileSchema
    })
    return 'mobile';
  } else {
    return 'unknown';
